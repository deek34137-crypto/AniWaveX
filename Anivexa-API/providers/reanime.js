...
async function resolveSeries(anilistId, ctx = {}) {
  ...
  // 1. Check direct anilist_id and cover_image in search results
  for (const [id, r] of candidates) {
    if (r?.anilist_id && Number(r.anilist_id) === Number(anilistId)) {
      const data = {
        animeId: id,
        title: r.title?.english || r.title?.romaji || id,
        anilistId: Number(anilistId),
        malId: r.mal_id ? Number(r.mal_id) : (malId ? Number(malId) : null),
  ...
  // 2. Check direct mal_id in search results
  if (malId) {
    for (const [id, r] of candidates) {
      if (r?.mal_id && Number(r.mal_id) === Number(malId)) {
        const data = {
          animeId: id,
          title: r.title?.english || r.title?.romaji || id,
          anilistId: Number(anilistId),
  ...
  // 3. Fallback: fetch detail pages
  const needsDetail = [...candidates.keys()].slice(0, 5);
  ...
  // 4. Fuzzy title matching on search candidates
  for (const [id, r] of candidates) {
    const candNorm = (r.title?.english || r.title?.romaji || "").toLowerCase().replace(/[^a-z0-9]/g, "");
    for (const q of queries) {
      const qNorm = q.toLowerCase().replace(/[^a-z0-9]/g, "");
      if (candNorm && qNorm && (candNorm === qNorm || candNorm.includes(qNorm) || qNorm.includes(candNorm))) {
        const data = {
          animeId: id,
          title: r.title?.english || r.title?.romaji || id,
          anilistId: Number(anilistId),
