...
import justanimeHandler            from "./providers/justanime.js";
...
export default {
  async fetch(request, env) {
    ...
    m = path.match(/^\/watch\/justanime\/(\d+)\/(sub|dub)\/justanime-(\d+)\/?$/);
    if (m) {
      const [, id, audio, ep] = m;
      return cachedWatch(
        `watch:justanime:${id}:${audio}:${ep}`,
        () => justanimeHandler.fetch(request)
      );
    }
    ...
    m = path.match(/^\/watch\/hianime\/(\d+)\/(sub|dub)\/hianime-(\d+)\/?$/);
    if (m) {
      const [, id, audio, ep] = m;
      return cachedWatch(
        `watch:hianime:${id}:${audio}:${ep}`,
        () => anikotoHandler.fetch(rewriteRequest(request, `/watch/anikoto/${id}/${audio}/anikoto-${ep}`))
      );
    }
    ...
    return json({
      ...
      routes: [
        ...
        "/watch/justanime/:id/sub|dub/justanime-:ep",
        "/watch/anikoto/:id/sub|dub/anikoto-:ep",
        "/watch/hianime/:id/sub|dub/hianime-:ep",
