...
function providerFns(anilistId, status, ctx) {
  return {
    ...
    justanime:  () => withCache(`epv:justanime:${anilistId}`,  status, () => justanimeEpisodes(anilistId, ctx)),
  };
}
...
export async function buildEpisodesWithCache(anilistId, media, anizip) {
  ...
  const [reanime, anikoto, animegg, anineko, animenosub, anibd, kaa, justanime] = await Promise.all([
    ...
    safe("justanime",  () => withCache(`epv:justanime:${anilistId}`,  status, () => justanimeEpisodes(anilistId, ctx))),
  ]);
  ...
  return {
    ...
    justanime:   justanime.ok   ? justanime.data   : { error: justanime.error,   stack: justanime.stack },
  };
}
