...
function getCountdown(targetEpochSeconds: number, nowMs: number) {
  const diff = targetEpochSeconds - Math.floor(nowMs / 1000);
  ...
  return (
    <div className="w-full flex flex-col gap-6">
      ...
      <div className="flex items-center gap-4 text-xs text-slate-400 px-1">
        ...
        {/* Live count */}
        {(() => {
          const liveCount = filteredAnime.filter(
            (a) => a.airingAt && getCountdown(a.airingAt, now).isLive
          ).length;
          return liveCount > 0 ? (
            <span className="flex items-center gap-1.5 text-red-400 font-bold">
              <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
              {liveCount} Airing Now
            </span>
          ) : null;
        })()}
      </div>
      ...
      {filteredAnime.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-5">
          {filteredAnime.map((anime) => {
            const countdown = getCountdown(anime.airingAt, now);
